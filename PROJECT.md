...
## 16. Известные особенности и решения

- **Пустой ответ LLM**: нестабильные модели (например `DeepSeek-V4-Flash`) иногда возвращают пустой
  `content`. Обрабатывается на трёх уровнях: ретраи в `LLMClient`, до 3 попыток в `verifier_node`,
  детерминированный fallback верификатора с уверенностью 0.75 (без роняния графа и без лишнего self-correction).
- **`is_blank` как INTEGER**: для корректности SQL генерация нормализует `false/true` → `0/1`.
- **Обрезанный SQL**: валидация проверяет баланс кавычек и скобок, что не даёт исполнить неполный запрос.
- **Self-correction**: при `low_confidence`/`failed` pipeline запускает повторный прогон с контекстом
  предыдущей попытки (`retry=True`).