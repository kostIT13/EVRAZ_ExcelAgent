...
Правила domain:
- prices: если в вопросе есть цена/лом/медь/латунь/аукцион/руб/тн (только вопросы о ценах)
- metrics: если в вопросе есть план/факт/отклонение/процент/шихта/бюджет/расход/
  силос/блок/запас/склад/остаток/состав/стоимость/масса
- ВАЖНО: если вопрос о расходе, составе шихты, запасах, бюджете или листах вида
  '2 блок'/'3 блок', то даже при упоминании 'поставщик' домен = metrics, а НЕ prices.
- generic: иначе
...
_METRICS_KEYWORDS = (
    "план", "факт", "отклонен", "шихт", "процент", "бюджет", "доля",
    "расход", "силос", "блок", "запас", "склад", "остаток", "состав",
    "стоимость", "масса", "участ", "утв",
)


def _heuristic_domain(question: str) -> str:
    q = (question or "").lower()
    # Metrics-ключи проверяем раньше prices: вопрос о расходе/составе шихты/
    # запасах может содержать слово 'поставщик', но относится к метрикам.
    if any(k in q for k in _METRICS_KEYWORDS):
        return "metrics"
    if any(k in q for k in ("цена", "медь", "латун", "бронз", "лом", "руб", "аукцион", "среднерыночн")):
        return "prices"
    return "generic"