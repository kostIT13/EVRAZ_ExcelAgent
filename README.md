...
- **Пустой ответ LLM**: нестабильные модели иногда возвращают пустой `content`. Обрабатывается на трёх
  уровнях: ретраи в `LLMClient`, до 3 попыток в `verifier_node`, детерминированный fallback верификатора
  (уверенность 0.75) — без роняния графа и без лишнего self-correction.
- **`is_blank` как INTEGER**: генерация SQL нормализует `false/true` → `0/1`.
- **Обрезанный SQL**: валидация проверяет баланс кавычек и скобок.
